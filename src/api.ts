...
export class VaultApi {
  ...
  // Filtered note query (tag / path_prefix), optionally pulling note bodies.
  // The dashboard uses this for both features: `{ tag: "status" }` with content
  // for the project cards, `{ tag: "todo" }` with content for the todo board.
  // We still re-check tags/paths client-side at the call site, so the board is
  // correct even if a server ignores an unknown query param.
